...
class GroupChat: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate{
    ...
    @IBOutlet var textSendView: UIView!
    var cellCount: Int = 0
...
//            self,
//            selector: #selector(GroupChat.viewWillEnterForeground(_:)),
//            name: UIApplication.willEnterForegroundNotification,
//            object: nil)
        //バックグランド
        NotificationCenter.default.addObserver(
            self,
            selector: #selector(GroupChat.viewDidEnterBackground(_:)),
...
//    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        var myData = self.dataSource[indexPath.row]
    ...
    func apiRequest() {
        ...
        API.requestHttp(POPOAPI.base.selectGroupChat, parameters: parameters,success: { [self] (response: [ApiGroupChatList]) in
    ...
    @IBAction func sendGroupChat(_ sender: Any) {
        ...
        API.requestHttp(POPOAPI.base.createGroupChat, parameters: parameters,success: { [self] (response: ApiStatus) in
                textFiled.text = ""
                self.comment = ""
                dataSource = []
                sendButton.isEnabled = true
