//
//  MultipleSecondViewController.swift
//  matchness
//
...
class MultipleSecondViewController: UIViewController, IndicatorInfoProvider, UITableViewDelegate , UITableViewDataSource, DZNEmptyDataSetSource, DZNEmptyDataSetDelegate {
    ...
    func apiRequest() {
        if status == 0 {
            presenter.apiMultiple(self.page_no, POPOAPI.base.myFoot)
            userDefaults.removeObject(forKey: "footprint")
        }
        if status == 3 {
            presenter.apiMultiple(self.page_no, POPOAPI.base.myLike)
            userDefaults.removeObject(forKey: "like")
        }
        if status == 6 {
            presenter.apiMylist(self.page_no, 2)
        }
    }
...
extension MultipleSecondViewController: MultipleOutput {
